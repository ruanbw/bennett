...
import { RsLoading, RsSpinner } from '../rs-spinner/exports'
...
const loadingDirective: Directive = {
  mounted(el, binding) {
    ...
    const instance = h(RsLoading, getOptions(binding))
    render(instance, el)
  ...
  updated(el, binding) {
    ...
    if (options && instance?.component?.props && instance.component.update) {
      try {
...
const spinningDirective: Directive = {
  mounted(el, binding) {
    const instance = h(RsSpinner, getOptions(binding))
    render(instance, el)
